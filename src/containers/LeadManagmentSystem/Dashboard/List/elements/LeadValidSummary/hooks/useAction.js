...
import { getLeadValidSummary } from '../../../../_repositories/repositories';
...
const colors =
  {
    opportunity: color.blue.main,
    Provisioning: color.green.main,
    Retired: color.primary.main,
    qualify: color.green.main,
    quote: color.yellow.main,
  } || color.general.main;
...
const useAction = () => {
  ...
  const [dotsActive, _setDotsActive] = useState(0);
  const setDotsActive = (val) => () => _setDotsActive(val);
  ...
  const fetchSummary = async () => {
    ...
    try {
      const result = await getLeadValidSummary({ params, withCancel: true });
      setLoading(false);
      setData(
  ...
  const dateOn = moment(filterDateSubmit[1]).format('DD MMMM YYYY');

  const countSlider = ['0', '1'];
