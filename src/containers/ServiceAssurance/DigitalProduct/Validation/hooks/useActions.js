...
const useAction = (props) => {
  ...
  useEffect(() => {
    if (isHaveAccess(feature, 'update_status_ticket_digital_product')) {
      ...
      setFailedAlert({
        ...
        onClose: () => router.push(route.digitalProduct('list')),
      });
    }
  }, [id]);
  ...
  const fetchDetail = async (refId) => {
    const validatePath =
      router.asPath === route.digitalProduct('gameqoo-validation', refId);
  ...
  const handleValidationSubmit = async (val) => {
    ...
    try {
      ...
      setSuccessAlert({
        ...
        onClose: () => router.push(route.digitalProduct('detail', id)),
      });
      return val;
    } catch (error) {
      setFailedAlert({
