...
const useAction = (props) => {
  ...
  const { id: orderNumber, productName: _productName } = router.query;
  ...
  const productName =
    {
      'smarthand neucentrix': 'smarthand',
    }[_productName] || _productName;

  const fetchUpdateStatus = async (data, { success, status }) => {
          ...
          res?.packages?.length > 0
            ? {
                ...
                price:
                  ...
                  `${rupiahFormat(res?.packages[0]?.price)}/${
                    res?.packages[0]?.paymentType
                  }`,
              }
            : {},
  ...
  const action = () => {
    ...
    const AMApproval =
      data?.worklog[data?.worklog?.length - 1].status.includes('am approval');
    const deliveryReturned =
      data?.worklog[data?.worklog?.length - 1].status.includes(
        'delivery returned',
      );

    if (
      data?.status === 'am approval' ||
      (data?.status === 'returned' && deliveryReturned)
    ) {
      if (isHaveAccess(feature, 'update_approvalRequest_am') && !AMApproval) {
        actions.push({
          // hideDivider: true,
          children: 'Return',
          variant: 'ghost',
