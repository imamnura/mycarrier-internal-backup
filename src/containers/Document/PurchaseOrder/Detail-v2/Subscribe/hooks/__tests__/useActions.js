...
describe('src/containers/Document/PurchaseOrder/Detail-v2/Subscribe/hooks/useActions', () => {
  afterEach(cleanup);
  ...
  test('run properly status delivery approval', async () => {
    getDetail.mockResolvedValue(resolvedData);
  ...
  test('run properly status delivery approval without privilage update', async () => {
    getDetail.mockResolvedValue(resolvedData);
    useState.mockImplementationOnce(() => [
      { status: 'delivery approval' },
      setData,
