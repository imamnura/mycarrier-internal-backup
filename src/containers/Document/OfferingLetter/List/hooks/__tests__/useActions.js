...
describe('src/pages/Document/OfferingLetter/List/hooks/useActions', () => {
  afterEach(() => {
    cleanup();
  });
  ...
  test('run properly', async () => {
    ...
    useRouter.mockReturnValue({
      pathname: route.offeringLetter('list'),
      push: jest.fn(),
    });
    getListOfferingLetter.mockResolvedValue({
      data: [{}],
      meta: { totalPage: 2, page: 1 },
    });
    // getFilterCompanyOptions.mockResolvedValue({ data: ['name1']});
    const props = { feature: ['read_list_all_offering_letter'] };
    ...
    act(() => {
      result.current.onClickNewOfferingLetter();
  ...
  test('fetch rejected', async () => {
    usePopupAlert.mockReturnValue({ setFailedAlert: jest.fn() });
    useRouter.mockReturnValue({ pathname: route.offeringLetter('list') });
    getListOfferingLetter.mockRejectedValue({ messsage: '' });
    // getFilterCompanyOptions.mockRejectedValue({ messsage: '' });
    const props = { feature: ['read_list_all_offering_letter'] };
