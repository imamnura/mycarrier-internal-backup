...
const useActions = (props) => {
  ...
  const fetchList = async (newPage) => {
    ...
    const validatePath = router.pathname === route.umb('list');

    if (!loadingTable && validatePath) {
      setLoadingTable(true);
      try {
        const { data, meta } = await getListUMB({ params, withCancel: true });
  ...
  const fetchOptionsCustomer = async () => {
    try {
      const { data } = await getListCustomerUMB();
  ...
  const onClickRowTable = (data) => {
    if (isHaveAccess(feature, 'read_detail')) {
      router.push(route.umb('detail', data.orderNumber));
    } else {
      setFailedAlert({
        message: "You don't have permission to read detail UMB.",
      });
    }
  };
