...
describe('src/containers/SMSA2P/UMB/list/hooks/useActions', () => {
  afterEach(() => {
    cleanup();
  });
  ...
  test('run properly', async () => {
    ...
    useRouter.mockImplementation(() => ({
      pathname: route.umb('list'),
      push: jest.fn(),
    }));

    getListUMB.mockResolvedValue({ data: [] });
    getListCustomerUMB.mockResolvedValue({ data: [] });
  ...
  test('fetch rejected', async () => {
    ...
    getListUMB.mockRejectedValue({ message: 'error' });
    getListCustomerUMB.mockRejectedValue({ message: 'error' });
  });
});
