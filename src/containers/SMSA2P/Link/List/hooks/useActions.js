...
const useActions = (props) => {
  ...
  const fetchList = async (newPage) => {
    ...
    const validatePath = router.pathname === route.link('list');

    if (!loadingTable && validatePath) {
      setLoadingTable(true);
      try {
        const { data, meta } = await getListLinkActivation({
          params,
          withCancel: true,
        });
  ...
  const fetchOptionsCustomer = async () => {
    try {
      const { data } = await getListCustomerLink();
  ...
  const onClickRowTable = (data) => {
    if (isHaveAccess(feature, 'read_detail')) {
      router.push(route.link('detail', data.orderNumber));
    } else {
      setFailedAlert({
        message: "You don't have permission to read detail Link.",
      });
    }
  };
