...
const useActions = (props) => {
  ...
  const fetchList = async (newPage) => {
    ...
    const validatePath = router.pathname === route.keyword('list');

    if (!loadingTable && validatePath) {
      setLoadingTable(true);
      try {
        const { data, meta } = await getListKeyword({
          params,
          withCancel: true,
        });
  ...
  const fetchOptionsCustomer = async () => {
    try {
      const { data } = await getListCustomerKeyword();
  ...
  const onClickRowTable = (data) => {
    if (isHaveAccess(feature, 'read_detail')) {
      router.push(route.keyword('detail', data.orderNumber));
    } else {
      setFailedAlert({
        message: "You don't have permission to read detail keyword.",
      });
    }
  };
