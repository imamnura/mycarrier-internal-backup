...
describe('src/containers/SMSA2P/Keyword/list/hooks/useActions', () => {
  afterEach(() => {
    cleanup();
  });
  ...
  test('run properly', async () => {
    ...
    useRouter.mockImplementation(() => ({
      pathname: route.keyword('list'),
      push: jest.fn(),
    }));

    getListKeyword.mockResolvedValue({ data: [] });
    getListCustomerKeyword.mockResolvedValue({ data: [] });
  ...
  test('fetch rejected', async () => {
    ...
    getListKeyword.mockRejectedValue({ message: 'error' });
    getListCustomerKeyword.mockRejectedValue({ message: 'error' });
  });
});
